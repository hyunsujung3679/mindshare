...
<ol>
  ...
  <li>ERD에 대한 고민</li>
  <ol>
    <li>현재 ERD에서 사용자 테이블은 자기 자신의 사용자 번호를 참조하는 자기참조 관계를 가지고 있으며, 이는 등록자와 수정자를 관리하기 위한 설계입니다</li>
    <li>현재 ERD에서 게시글 테이블은 사용자 테이블의 사용자 번호를 외래키로 참조하여 게시글의 등록자와 수정자 정보를 관리하는 구조로 설계로 설계되어 있습니다.</li>
    <li>게시글에서 작성자 또는 수정자의 아이디를 조회하려면 JOIN이 발생하게 됩니다.</li>
    <li>지속적이고 잦은 JOIN이 발생하는 상황이기 때문에 게시글 테이블에 '작성자'라는 컬럼을 별도로 생성해야할지에 대한 고민이 있었습니다.</li>
    <li>현재는 아주 간단한 프로젝트이기 때문에 1개의 JOIN으로 인한 속도 문제가 발생하지 않아서 '작성자'라는 컬럼을 별도로 생성하지 않았습니다.</li>
    <li>이 글을 작성하고 있는 시점에 생각해봤을 때, 개발 초기로 돌아간다면 '작성자'라는 컬럼을 별도로 생성할 것 같습니다.</li>
  </ol>
  <br/>
  <li>네이밍에 대한 고민</li>
  <ol>
